...
shiny::shinyServer(function(input, output, session) {
  ...
  shinyFiles::shinyDirChoose(input, "json_dir", roots = volumes, session = session)

  output$json_dir_path <- shiny::renderPrint({
    if (length(parseDirPath(volumes, input$json_dir)) == 0) {
      cat("No directory has been selected (json_dir)")
    } else {
      shinyFiles::parseDirPath(volumes, input$json_dir)
    }
  })

  shiny::observeEvent(input$load_json_btn, {
    shinyjs::disable("tabs")

    json_dir_parsed <- shinyFiles::parseDirPath(volumes, input$json_dir)

    if (length(json_dir_parsed) == 0) {
      shinyjs::enable("tabs")
      shiny::showNotification("Please select a valid JSON directory.", type = "error", closeButton = TRUE)
      return()
    }
    ...
    tryCatch(
      {
        ...
        json_files <- list.files(json_dir_parsed, pattern = "\\.json$", recursive = TRUE, full.names = TRUE)

        if (length(json_files) == 0) {
          shinyjs::enable("tabs")
          shiny::showNotification("No JSON files found in the selected directory.", type = "error", closeButton = TRUE)
          return()
        }
        ...
        shiny::showModal(shiny::modalDialog(
          title = "Processing",
          "Loading JSON files... Please wait.",
          footer = NULL,
          easyClose = FALSE
        ))
        ...
        # Show progress
        progressr::withProgressShiny(message = "Loading JSON files...", {
          p <- progressr::progressor(along = json_files)
          ...
          for (file_path in json_files) {
            ...
            # Derive sample name from filename (remove .json and fastp prefix/suffix if present)
            # Simple approach: remove extension
            sample_name <- tools::file_path_sans_ext(file_name)
            # You might want more sophisticated name cleaning here if needed
            sample_names <- c(sample_names, sample_name)

            # Read JSON
            json_data <- jsonlite::fromJSON(file_path, simplifyVector = FALSE)

            results[[sample_name]] <- json_data
          }
        })
        ...
        # Extract values
        result_list <- extract_values(results, fix_overflow = FALSE)
        result_values$result_list <- result_list
        ...
        shiny::showModal(shiny::modalDialog(
          title = "Complete",
          paste("JSON loading is complete. Loaded", length(json_files), "files."),
          easyClose = TRUE,
          footer = shiny::modalButton("Close")
        ))
      },
      error = function(e) {
        shiny::removeModal()
        shinyjs::enable("tabs")
        shiny::showNotification(paste("Error loading JSON:", e$message), type = "error")
      }
    )
  })
  ...
  result_summary <- shiny::reactive({
    result_values$result_list[[1]]
  })
  filtering_result <- shiny::reactive({
    result_values$result_list[[2]]
  })
  duplication <- shiny::reactive({
    result_values$result_list[[3]]
  })
  insert_size <- shiny::reactive({
    result_values$result_list[[4]]
  })
  adapter_cutting <- shiny::reactive({
    result_values$result_list[[5]]
  })
  read1_before_filtering <- shiny::reactive({
    result_values$result_list[[6]]
  })
  read2_before_filtering <- shiny::reactive({
    result_values$result_list[[7]]
  })
  read1_after_filtering <- shiny::reactive({
    result_values$result_list[[8]]
  })
  read2_after_filtering <- shiny::reactive({
    result_values$result_list[[9]]
  })
  ...
  output$read1_before_filtering_qualitycurves <- plotly::renderPlotly({
    ...
    tmp <- read1_before_filtering()[[input$selected_sample]]$quality_curves %>%
      ...
      tidyr::pivot_longer(cols = -position)
